...
在 IContextAssembler 返回的数据块基础上，Application 层必须插入强硬的元指令（Meta-Prompt），格式如下：

```
【系统指令】
你必须严格遵守以下规则：
1. **所有生成内容必须基于下方提供的【设定数据】**，严禁编造文档中未提及的设定。
2. **如果用户输入与【设定数据】冲突，以【设定数据】为准**，并指出冲突点。
3. **生成前必须进行逻辑校验**：检查角色状态（如生死）、物品归属、时间线等关键事实。
4. **若发现数据矛盾或缺失，立即停止生成并返回错误码**（如：`ERROR_CONFLICT: 角色[张三]在上一章已死亡，无法执行[说话]动作`）。
```

✅ 优势：将"约束力"从隐式转为显式，迫使 AI 进入"校验模式"而非"自由创作模式"。

## 十五、结构化 Prompt 注入规范

改变现有的 StringBuilder 拼接方式，采用 **分层标记 + 强制分隔符**：
...
## 角色档案
- {character_name}: {status} | {key_traits} | {current_location}

## 当前章节上下文
{current_chapter_summary}

【生成要求】
1. **输出格式**：严格按 `[动作][对话][环境]` 三段式结构。
2. **禁止行为**：不得添加设定外的角色/物品/规则。
3. **逻辑检查点**：生成后需确认 {character_name} 的 {status} 未发生非法变更。
```

✅ 优势：通过结构化分隔符（如 ##、-、[]）提升 AI 对关键数据的注意力，减少信息混淆。

## 十六、动态事实记忆库机制

在现有 MemorySnapshot 基础上增加 **实时事实校验层**：

### 生成前校验
从 MemorySnapshot 提取关键事实（如角色状态、物品位置），生成 **事实向量**：
```
{character_name}|{status}|{location}|{inventory}
```

### 生成后验证
将新生成内容与事实向量比对，若检测到冲突（如 status: alive → dead 无过渡），触发以下流程：
- **自动回滚**：丢弃当前生成内容
- **强制修正**：插入逻辑过渡句（如"他中了剧毒，挣扎着说出最后一句话"）
- **用户告警**：返回 `WARNING_TRANSITION: 需补充[中毒]情节以支持[死亡]结果`

✅ 优势：将"异步纠错"改为"实时阻断"，避免错误滚雪球式传播。

## 十七、关键场景的思维链（CoT）模板

对高风险操作（如角色死亡、重大转折）启用 **强制推理链**：

```
【思维链指令】
生成前必须完成以下推理：
1. 检查角色当前状态：{status}  
2. 验证触发条件：{event} 是否满足 {status} 变更要求  
3. 若状态变更，确认存在过渡情节：{transition_scenes}  
4. 输出推理结果：[通过/拒绝] + 原因
```

✅ 优势：将隐性逻辑显性化，减少 AI 的"直觉式错误"。

## 十八、实施建议

### 分层启用策略
- **常规续写**：使用 **元指令 + 结构化 Prompt**
- **关键情节**：追加 **思维链模板 + 动态事实校验**

### 监控指标
- `constraint_violation_rate`：统计因违反规则被拦截的生成次数
- `fact_conflict_rate`：检测事实冲突的频率
- 根据指标动态调整 **元指令严格度**

### Fallback 机制
当检测到高频冲突时，自动降级为 **"安全模式"**：
- 仅允许生成中性描述（如"他沉默地离开"）
- 禁止状态变更类操作
- 强制用户确认关键决策
