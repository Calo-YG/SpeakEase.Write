...
public sealed class CreateOutlineNodeTool(IServiceScopeFactory scopeFactory) : IToolExecutor
{
    ...
    public static readonly ToolDefinition ToolDefinition = new()
    {
        ...
        Function = new FunctionDefinition
        {
            ...
            Description = """
创建大纲节点。大纲系统支持三层结构，使用 stage_type 标记层级，通过 parent_node_id 建立树状关系：
...
""",
            Parameters = new FunctionParameters
            {
                ...
                Properties = new Dictionary<string, ParameterSchema>
                {
                    ...
                    {
                        Type = "string",
                        Description = "层级/阶段类型（必填）。枚举值: book(全书总纲) / volume(卷大纲) / act(发展章) / climax(高潮章) / resolution(结局章)",
                ...
                Required = ["work_id", "title", "stage_type"]
            }
        }
    };
    ...
    public async Task<ToolResult> ExecuteAsync(string arguments, CancellationToken ct)
    {
        ...
        var stageType = args.GetString("stage_type", required: true);
        ...
        return ToolResult.Ok($"大纲节点「{title}」已创建，层级: {stageType}，ID: {entity.Id}，序号: {entity.Sequence}");
    }
